...
from typer_tutorial.cli import (
    DEFAULT_NAMES,
    FORMAL_GREETING,
    FORMAL_GREETING_STYLE,
    GREETING,
    GREETING_STYLE,
    SILLY_MESSAGE,
    SILLY_NAMES,
...
# GREETING_WITH_DEFAULT_NAME = f"{STYLED_GREETING} {DEFAULT_NAMES[0]}!"
...
def test_print_default_greeting():
    ...
    assert result.stdout.startswith(STYLED_GREETING)

    actual_name = result.stdout.replace(STYLED_GREETING, '').split('!')[0].strip()
    assert actual_name in DEFAULT_NAMES
